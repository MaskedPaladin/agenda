...
public class GameFuncs{
    ...
    int maxmembers = 4;
    Member[] enemies = new Member[maxmembers];
    Member[] allied = new Member[maxmembers];
    void initialize(){
    ...
    public void listar(){
        for (int i = 0; i < maxmembers; i++) {
            System.out.println("Name: "+allied[i].name+" HP: "+allied[i].hp+" MANA: "+allied[i].mana+" STAMINA: "+allied[i].stamina+" (STR: "+allied[i].str+" INT: "+allied[i].intel+" DEX: "+allied[i].dex+" CON: "+allied[i].con+" WIS: "+allied[i].wisp+" MEDIA: "+allied[i].media+")");
        }
    }
}